...
import {
  ...
  extractFrontmatter,
  extractDirectives,
} from '@markdownkit/remark-mdd/validator'
...
test('Invalid version format should produce error', () => {
  ...
  assertErrorCode(
    ...
    'Should have INVALID_VERSION_FORMAT error',
  )
})
...
test('Invalid currency format should produce error', () => {
  ...
  assertErrorCode(
    ...
    'Should have INVALID_CURRENCY_FORMAT error',
  )
})
...
test('Missing required directive should produce error', () => {
  ...
  assertErrorCode(
    ...
    'Should have MISSING_REQUIRED_DIRECTIVE error',
  )
})
...
test('Unknown semantic class should produce warning', () => {
  ...
  assertErrorCode(
    ...
    'Should have UNKNOWN_SEMANTIC_CLASS warning',
  )
})
...
test('Invoice without invoice-number should produce error', () => {
  ...
  const error = result.errors.find(
    (e) => e.code === 'MISSING_REQUIRED_FIELD' && e.location.field === 'invoice-number',
  )
...
test('Valid document should pass in strict mode', () => {
  ...
  assert(
    ...
    `Valid document should have no errors in strict mode (errors: ${result.errors.map((e) => e.code).join(', ')})`,
  )
})
...
# Test`
  ...
  assertEquals(
    result.errors.filter((e) => e.code === 'INVALID_DATE_FORMAT').length,
    0,
